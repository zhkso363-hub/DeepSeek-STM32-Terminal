...
/* 全局变量定义（extern 声明见 bsp_usart.h） */
volatile uint8_t g_emotion_tag        = 0;
volatile uint8_t g_emotion = 0;
volatile uint8_t g_data_len           = 0;
volatile uint8_t g_packet_ready       = 0;
char g_ai_response[RX_BUF_SIZE] = {0};

/**
  * @brief  USART1 中断服务函数 — 5 状态帧解析
  *   状态 0: 寻找帧头 0xAA
  *   状态 1: 读取情感标签
  *   状态 2: 读取数据长度
  *   状态 3: 接收正文
